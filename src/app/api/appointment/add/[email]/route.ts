...
export const POST = async (
  ...
  try {
    ...
    const patient = await prisma.patient.findMany({
      where: { name: patientName, doctorEmail: email },
    });
    if (!patient.length) {
    ...
    const timeAppointed = await prisma.appointment.findFirst({
      where: { date, time, doctorEmail: email },
    });
    ...
    overlappingAppointment.forEach((existingAppointment) => {
      ...
      const existingEndTime = addMinutes(
        existingStartTime,
        parseInt(existingAppointment.duration)
      );

      if (
        (newAppointmentStartTime >= existingStartTime &&
          newAppointmentStartTime < existingEndTime) ||
        (newAppointmentEndTime > existingStartTime &&
          newAppointmentEndTime <= existingEndTime) ||
        (existingStartTime >= newAppointmentStartTime &&
          existingStartTime < newAppointmentEndTime)
      ) {
